...
def main():
    ...
    default_analysis_root = None
    candidate_roots = [
        os.path.abspath(os.path.join(script_dir, "..")),
        os.path.abspath(os.path.join(script_dir, "..", "05_benchmark_analysis")),
        os.path.abspath(os.path.join(script_dir, "..", "..", "05_benchmark_analysis")),
    ]
    for root in candidate_roots:
        if os.path.exists(os.path.join(root, "scripts", "10_stage_pas_match_quantify.R")) and os.path.exists(
            os.path.join(root, "scmapa", "parameterized_scmapa_analysis.R")
        ):
            default_analysis_root = root
            break
    if default_analysis_root is None:
        default_analysis_root = os.path.abspath(os.path.join(script_dir, ".."))

    parser = argparse.ArgumentParser(description="Run raw benchmark and/or pair-level inputs on auto-selected top samples")
    ...
    parser.add_argument(
        "--prepare-only",
        action="store_true",
        help="Only prepare raw_pair_manifest/raw_pair_inputs/raw_qc for downstream pipelines; skip performance and raw_dapars2_format stages.",
    )
    ...
    all_raw_tools = sorted(
        [
            name
            for name in os.listdir(raw_bam_root)
            if os.path.isdir(os.path.join(raw_bam_root, name))
        ]
    )
    dapars_tools = [t for t in ["scmapa", "dapars2"] if t in all_raw_tools]
    standard_tools = [t for t in all_raw_tools if t not in dapars_tools]
    ...
    prepare_meta_rows = [
        {
            ...
            "raw_tools": ",".join(all_raw_tools),
            "standard_tools": ",".join(standard_tools),
            "dapars_tools": ",".join(dapars_tools),
        }
    ]
    write_tsv(
    ...
    for sample in selected_samples:
        ...
        required = [gt_mtx, gt_pas]
        for tool in standard_tools:
            required.append(os.path.join(raw_bam_root, tool, sample, "pas_counts.tsv"))
            required.append(os.path.join(raw_bam_root, tool, sample, "pas.bed"))
        if not args.prepare_only and "scmapa" in dapars_tools:
            required.append(scmapa_src)
        if not args.prepare_only and "dapars2" in dapars_tools:
            required.append(dapars2_src)
        missing = [p for p in required if not os.path.exists(p)]
        if missing:
            for m in missing:
                msg = f"missing_input:{m}"
                perf_failures.append({"sample": sample, "pair_label": "ALL", "tool": "all", "stage": "input_check", "message": msg})
        ...
        for tool in standard_tools:
            if tool in per_sample_transform[sample]:
                continue
            tool_mtx = os.path.join(raw_bam_root, tool, sample, "pas_counts.tsv")
            t_name, t_fn, hits, total = infer_barcode_transform(tool_mtx, all_gt_barcodes)
            per_sample_transform[sample][tool] = (t_name, t_fn)
            print(f"[{sample}] {tool} barcode transform: {t_name} ({hits}/{total} matched)")
        ...
        for pair in sample_pairs:
            ...
            if not selected_cells:
                ...
                perf_failures.append({"sample": sample, "pair_label": pair_label, "tool": "all", "stage": "prepare_pair", "message": msg})
            ...
            for tool in standard_tools:
                tool_mtx = os.path.join(raw_bam_root, tool, sample, "pas_counts.tsv")
                tool_pas = os.path.join(raw_bam_root, tool, sample, "pas.bed")
                tool_pair_expr = os.path.join(raw_pair_inputs_dir, tool, sample, pair_label, "pd_pas_counts.tsv")
                tool_pair_pas = os.path.join(raw_pair_inputs_dir, tool, sample, pair_label, "pd_pas.bed")
                transform_name, transform_fn = per_sample_transform[sample][tool]
                try:
                    ...
                    pd_bed_kept = filter_tool_pas_bed(tool_pas, pd_keep_set, tool_pair_pas)
                    qc_rows.append(
                        {
                            "sample": sample,
                            "pair_label": pair_label,
                    ...
                    msg = f"{tool}_prepare_failed:{e}"
                    perf_failures.append({"sample": sample, "pair_label": pair_label, "tool": tool, "stage": "prepare_pair", "message": msg})
                    dapars_failures.append({"sample": sample, "pair_label": pair_label, "tool": tool, "stage": "prepare_pair", "message": msg})
                    continue
