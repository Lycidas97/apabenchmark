...
def read_bed_like_file(file_path):
    ...
    def looks_like_data_row(row):
        return (
            is_int_like(row.iloc[1])
            and is_int_like(row.iloc[2])
            and str(row.iloc[5]).strip() in {"+", "-"}
        )

    # Allow non-standard BED with optional header/comments by skipping leading non-data lines.
    while len(df) > 0 and not looks_like_data_row(df.iloc[0]):
        df = df.iloc[1:]

    if len(df) == 0:
        raise ValueError(f"No BED data rows found in {file_path}.")
    ...
    if df['start'].isna().any() or df['end'].isna().any():
        raise ValueError(f"Found non-numeric start/end rows in {file_path}.")

    if not df['strand'].isin(['+', '-']).all():
        raise ValueError(f"Found invalid strand values in {file_path}; expected only '+' or '-'.")
