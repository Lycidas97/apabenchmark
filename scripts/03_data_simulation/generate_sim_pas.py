...
MAX_PAS_PER_TE = 5
MIN_GAP_LEN = 200
PAS_PATH = "../../data/int_data/annotations/mouse_integrated_pas.bed"
# Read PAS Annotation
...
for gene_number in GENE_NUMBERS:
    for rep in range(1, 4):
        sample_bed = f"../../data/sim_data/sim_pas/mm10_sim_pas_gn{gene_number}_rep{rep}.bed"
        os.makedirs(os.path.dirname(sample_bed), exist_ok=True)
        
        # calculate the number of each gene type based on the ratio
        single_pas_gene_num = int(gene_number * 0.2)
        diff_apa_te_num = int(gene_number * 0.4)
        ...
        diff_apa_te_pas = (diff_apa_te_pas.groupby('exon_id').apply(lambda x: x.sample(np.random.randint(2, min(MAX_PAS_PER_TE+1, len(x)+1))).reset_index(drop=True)))
        ...
        non_diff_apa_te_pas = (non_diff_apa_te_pas.groupby('exon_id').apply(lambda x: x.sample(np.random.randint(2, min(MAX_PAS_PER_TE+1, len(x)+1))).reset_index(drop=True)))
