#!/usr/bin/env python3
"""Render PAS quantification metric-by-protocol panels for sim data performance."""
...
def render_metric(df, metric: str) -> None:
    output_name = f"pas_quantification_{metric}_by_protocol.pdf"
    ...
    for idx, (protocol, ax) in enumerate(zip(protocol_order, axes)):
        ...
        ax.set_ylabel(METRIC_LABELS[metric] if idx == 0 else "", labelpad=2)
        ax.set_xlabel("")
    ...
    fig.legend(
        ...
        labels,
        loc="upper center",
        bbox_to_anchor=(0.5, cfg_float("layout.pas_quant_by_protocol_legend_y", 0.95)),
...
def main() -> None:
    ...
    apply_runtime_rcparams()
    df = load_metric_domain("pas_quantify_performance")
